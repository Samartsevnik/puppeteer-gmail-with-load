...
const emailSender = {
  ...
  writeNewEmail: async (page, { index, subject, email, message }) => {
    ...
    const $messageInput = `[aria-label*="Тело письма"][role=textbox]`;
    ...
    await page.click('[data-tooltip="Прикрепить файлы"]')
    let fileChooser = await futureFileChooser;
    await page.waitFor(delayBetweenSteps);

    await fileChooser.accept(['/home/gemini/Downloads/Homework .docx']);
    await page.waitFor(delayBetweenSteps);
    await page.waitFor(delayBetweenSteps);

    let futureFileChooser1 = page.waitForFileChooser();
    await page.click('[data-tooltip="Прикрепить файлы"]')
    let fileChooser1 = await futureFileChooser1;
    await fileChooser1.accept(['/home/gemini/Downloads/DOM шпаргалка.docx']);
    await page.waitFor(delayBetweenSteps);
